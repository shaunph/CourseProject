...
findAfter throws errors if startIndex is larger than sourceBuffer.length or is negative
*/
findAfter = function (sourceBuffer, subBuffer, startIndex) {
    if (startIndex<sourceBuffer.length){
        if (startIndex>=0) {
            ...
            throw "Error in bufferOps.js.findAfter: startIndex is negative";
        }
    } else {
        throw "Error in bufferOps.js.findAfter: startIndex is larger than sourceBuffer length";
    }
};
...
split = function (bufferMain,bufferDelimiter) {
    ...
    if (bufferMain.length==0) {
        ...
        var splitBuffer = [];
        var iPos = 0;
        var iNextPos = find(bufferMain,bufferDelimiter,iPos);
        while (iNextPos != -1) {
...
extractFileFromResponse = function (dataBuffer) {
        ...
        if (sepEndIndex==-1) {
            ...
            throw "Error in bufferOps.js.extractFileFromResponse: Malformed Input";
        }
