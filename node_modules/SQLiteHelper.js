...
*/
function accessDB(sql, executionArgs, callback) {
	...
	db.open(dbLocation, function(error) {
        ...
        // execute the sql statement and the callback when the sql query returns
        if(executionArgs == null)
            db.execute(sql, callback);
        else
            db.execute(sql, executionArgs, callback);
	});
...
exports.addTask = function(taskObj, callback) {
	...
	accessDB(sql, null, function(error, rows) {
        ...
        // this for loop disallows the add if the given task is already in the database
		...
		db.execute(sql, [taskObj.getTaskName(), taskObj.getDescription(),
			...
			function(error, rows) {
                ...
                // task added successfully, now execute callback, if there is one
...
exports.addUser = function(userEmail, userNickname, userPassword, callback) {
	...
	accessDB(sql, [userEmail, userNickname], function(error, rows) {
        ...
        // disallows the add if the user is already in the database
        if(rows.length != 0) {
            ...
            db.execute(sql, [userEmail, userNickname, userPassword],
                function(error, rows) {
                    ...
                    // user added successfully, now execute callback, if there is one
                    if(callback != undefined) { callback({status:0, detail:error}); }
                }
            );
        }
...
exports.addComment = function(commentText, commentTaskid, commenterEmail, callback) {
	...
	accessDB(sql, [commenterEmail], function(error, rows) {
        ...
        // stops a comment from being added if the user who's commenting is not in the database
        if(rows.length == 0) {
        ...
        db.execute(sql, [commentTaskid], function(error, rows) {
            ...
            // stops a comment from being added if the taskid given is not in the database
            if(rows.length == 0) {
            ...
            db.execute(sql,
                [commentText, commentTaskid, commenterEmail], function(error, rows) {
                    ...
                    // comment added successfully, now execute callback, if there is one
                    if(callback != undefined) { callback({status:0, detail:error}); }
                }
            );
        });
...
exports.getTable = function(tableName, callback) {
	...
	db.open(dbLocation, function(error) {
		...
		db.execute(sql, function(error, rows) {
            ...
            // execute callback, passing in the retrieved table
