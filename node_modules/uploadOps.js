...
parseURLEncoded = function(requestURL) {
    //requestURL = "/dir1/page.js?foo=1&bar=2"
    //url.parse(requestURL,true).search = "?foo=1&bar=2"
    ...
    if (urlSearch.length > 0) {
        ...
        console.log(urlSliceSplit);
        var objectData = [];
        var singleValue = [];
        for (var d in urlSliceSplit) {
            singleValue = urlSliceSplit[d].split("=");
            if (singleValue.length != 2) {
                throw "URL Parsing error, invalid pair"
            } else {
                objectData[singleValue[0]] = urlDecode(singleValue[1]);
            }
        }
        return objectData;
    }
...
urlDecode = function(encodedURL) {
    var decodedURL = encodedURL.replace("+"," ");
    return decodedURL;
}
