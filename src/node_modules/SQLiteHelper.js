...
var queryGetRows = function (sql, callback, qargs) {
    ...
    errorHandling = function(error, rows) {
        if(error) {
            writeLog(error);
            safeCallback(-2, undefined, error, rows, callback); 
            throw new Error(error.message);
...
exports.addTask = function(taskObj, callback) {
    ...
    accessDB(sql, null, function(error, rows) {
        if(error) {
            writeLog(error);
            if (callback != undefined) { callback({status:-2, detail:error}); } //callback parameter for db error is -2
            return -2;
        }
            ...
            for(i = 0; i < rows.length; i++) {
                if (rows[i].taskName.toLowerCase() ==
                    ...
                    writeLog("func: addTask, task " + taskObj.getTaskName() +
                            " already exists.");
                    if (callback != undefined) {
                        callback({status:i, detail:error}); //parameter is used to get the row where the like-named task is entered
                    }
                    return -2;
                }
            }
        ...
        accessDB(sql, [taskObj.getTaskName(), taskObj.getDescription(), 
            ...
            function(error, rows) {
                ...
                if (callback != undefined) { callback({status:-1, detail:error}); } //-1 = success parameter that the task was added to the db
            }
        );
    });
}
...
/**
    Parameter1: name of estimate to delete. (String)
    Parameter2: callback. (function)

    removes estimate with given taskId if it exists.
*/
exports.removeEstimate = function (theTaskid, callback) {
    queryGetRows("DELETE FROM estimate WHERE taskid = ?", callback, [theTaskid]);
}
...
exports.getRecentActivity = function(callback) {
    var sql = "SELECT user.nickname AS UserName, feed.topic as Topic, feed.created as Created"
        ...
        + " UNION"
        + " SELECT user AS author, \"New Task:\" || taskName AS topic, created"
            + " FROM task"
        + " UNION"
