...
exports.addTask = function(taskObj, callback) {
    ...
    accessDB(sql, null, function(error, rows) {
        if(error) {
            writeLog(error);
            if (callback != undefined) { callback(-2); } //callback parameter for db error is -2
            return -2;
        }
            ...
            for(i = 0; i < rows.length; i++) {
                if  (rows[i].taskName.toLowerCase() ==
                    ...
                    writeLog("func: addTask, task " + taskObj.getTaskName() +
                            " already exists.");
                    if (callback != undefined) {
                        callback(i); //parameter is used to get the row where the like-named task is entered
                    }
                    return -2;
                }
            }
        ...
        sql = "INSERT INTO task " +
            "(taskName, description, timeLeft, timeSpent, priority, status, user, date) " +
            "VALUES (?,?,?,?,?,?,?,?)";

        accessDB(sql, [taskObj.getTaskName(), taskObj.getDescription(), taskObj.getTimeLeft(),
            taskObj.getTimeSpent(), taskObj.getPriority(), taskObj.getStatus(), taskObj.getUser(),
            taskObj.getDate()],
            function(error, rows) {
                if(error) {
                    writeLog(error);
                ...
                if (callback != undefined) { callback(-1); } //-1 = success parameter that the task was added to the db
            }
        );
    });
}
