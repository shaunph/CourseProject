...
*/
module.exports.isMatch = function (bfr1,bfr2) {
    var i;
...
*/
module.exports.find = function (sourceBuffer, subBuffer) {
    var srcIndex, slicedBuffer;
    ...
    if (subBuffer.length>sourceBuffer.length) {
        ...
        for(srcIndex=0;srcIndex<sourceBuffer.length-subBuffer.length+1;srcIndex++) {
            slicedBuffer = sourceBuffer.slice(srcIndex,srcIndex+subBuffer.length);
            if (this.isMatch(slicedBuffer,subBuffer)) {
                return srcIndex;
            }
        }
        return -1;
...
*/
module.exports.findAfter = function (sourceBuffer, subBuffer, startIndex) {
    if (startIndex<sourceBuffer.length){
        if (startIndex<0) {
            startIndex=0;
        }
        var foundIndex = this.find(sourceBuffer.slice(startIndex),subBuffer);
        if (foundIndex === -1) {
            return -1;
        } else {
...
*/
module.exports.join = function (bufferArray) {
    var i;
...
module.exports.split = function (bufferMain,bufferDelimiter) {
    ...
    if (bufferMain.length===0) {
        ...
        var iNextPos = this.find(bufferMain,bufferDelimiter,iPos); //iNextPos is the
        ...
        while (iNextPos !== -1) {
            ...
            if (iPos<bufferMain.length) {
                iNextPos = this.findAfter(bufferMain,bufferDelimiter,iPos);
            } else {
                iNextPos = -1;
            }
        }
