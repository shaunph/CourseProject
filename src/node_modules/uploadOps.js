...
*/
module.exports.parseMultipartFormdata = function (dataBuffer) {
    ...
    */
    ...
    for (i = 1; i < splitBuffer.length - 1; i++) {
        ...
        var startIndex = bops.find(splitBuffer[i], crlfcrlfBuffer);
        if (startIndex === -1) {
...
// parseCookies accepts as input the headers (request.headers) of a request
// and returns the values of the cookies. 
module.exports.parseCookies = function (headers) {
    var c;
    var allCookies = headers.cookie;
    if (allCookies === undefined) {
        ...
        var splitCookie;
        for (c in allCookies) {
            if (allCookies.hasOwnProperty(c)) {
                splitCookie = allCookies[c].split('=');
