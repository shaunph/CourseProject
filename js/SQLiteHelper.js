 ...
 */
exports.updateTask = function(taskObj, callback) {
	
	var sql = "SELECT * FROM task WHERE taskid = ?";
	
	accessDB(sql, [taskObj.getId()], function(error, rows) {
		if(error) {
			writeLog(error);
			if (callback != undefined) {
				callback({status:-2, detail:error});
				return -2;
			}
		}
		
		sql = "UPDATE task " +
			"SET taskName = ?, description = ?, priority = ?, status = ?)";
		
		db.execute(sql, [taskObj.getTaskName(), taskObj.getDescription(), 
			taskObj.getPriority(), taskObj.getStatus()], 
			function(error, rows) {
			if(error) {
				writeLog(error);
				if (callback != undefined) {
